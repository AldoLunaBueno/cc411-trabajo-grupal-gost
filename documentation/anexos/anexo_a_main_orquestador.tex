\section{Código del Orquestador Principal (Main.java)}
\label{anexo:main}

Este anexo contiene el código del orquestador principal del sistema (\texttt{Main.java}). Su propósito consiste en simular un entorno completo de transmisión donde se inicializan los parámetros oficiales del estándar GOST R 34.10, se generan llaves asimétricas, se calcula el hash criptográfico empleando Streebog y se procesa el flujo completo de firma y validación.

\begin{lstlisting}[language=Java, caption={Clase Main.java - Integración de componentes y flujo de firma/verificación}]
package uni;
